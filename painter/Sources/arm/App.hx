		...
		if (menuCategory == 0) {
			...
			if (ui.button("Manual...", Left)) {
				if (kha.System.systemId == "Windows") {
					Krom.sysCommand('explorer "https://phasor.io/manual"');
				}
				else if (kha.System.systemId == "Linux") {
					Krom.sysCommand('xdg-explorer "https://phasor.io/manual"');
				}
				else {
					Krom.sysCommand('open "https://phasor.io/manual"');
				}
			}
			...
			if (ui.button("About...", Left)) {
				...
				var msg = "Phasor.io - v" + version + " (" + date + ") - git " + sha + "\n";
