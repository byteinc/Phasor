...
float traceAO(vec3 origin, vec3 normal, sampler3D voxels_1)
{
    ...
    vec3 c2 = (o1 - o2) * 0.5;
    vec3 param = normal;
    float _316 = traceConeAO(voxels_1, origin, param, 0.557851731777191162109375, 3.4641015529632568359375);
    float col = _316;
    vec3 param_1 = mix(normal, o1, vec3(0.5));
    float _321 = traceConeAO(voxels_1, origin, param_1, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _321;
    vec3 param_2 = mix(normal, o2, vec3(0.5));
    float _328 = traceConeAO(voxels_1, origin, param_2, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _328;
    vec3 param_3 = mix(normal, -c1, vec3(0.5));
    float _336 = traceConeAO(voxels_1, origin, param_3, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _336;
    vec3 param_4 = mix(normal, -c2, vec3(0.5));
    float _344 = traceConeAO(voxels_1, origin, param_4, 0.557851731777191162109375, 3.4641015529632568359375);
    col += _344;
    return (col / 5.0) * 0.89999997615814208984375;
}
...
float integrateEdge(vec3 v1, vec3 v2)
{
    ...
    {
        _369 = theta / sin(theta);
    }
    else
    {
        _369 = 1.0;
    }
    float res = cross(_362, _363).z * _369;
    return res;
}
...
float traceShadow(sampler3D voxels_1, vec3 origin, vec3 dir)
{
    vec3 param = dir;
    float _292 = traceConeAO(voxels_1, origin, param, 0.16799999773502349853515625, 5.0);
    return _292;
}

vec3 sampleLight(vec3 p, vec3 n, vec3 v, float dotNV, vec3 lp, vec3 lightCol, vec3 albedo, float rough, float spec, vec3 f0, sampler3D voxels_1, vec3 voxpos)
{
    vec3 ld = lp - p;
    vec3 l = normalize(ld);
    ...
    float _974 = ltcEvaluate(param, param_1, param_2, param_3, param_4, param_5, param_6, param_7, param_8);
    float ltcspec = _974;
    ...
    float _998 = ltcEvaluate(param_9, param_10, param_11, param_12, param_13, param_14, param_15, param_16, param_17);
    float ltcdiff = _998;
    vec3 direct = (albedo * ltcdiff) + vec3((ltcspec * spec) * 0.0500000007450580596923828125);
    direct *= attenuate(distance(p, lp));
    direct *= lightCol;
    direct *= (1.0 - traceShadow(voxels_1, voxpos, l));
    return direct;
}
...
void main()
{
    ...
    {
        _1194 = octahedronWrap(g0.xy);
    }
    n = vec3(_1194.x, _1194.y, n.z);
    n = normalize(n);
    ...
    vec3 voxpos = p / vec3(1.0);
    envl *= (1.0 - traceAO(voxpos, n, voxels));
    fragColor = vec4(envl.x, envl.y, envl.z, fragColor.w);
    vec3 _1352 = fragColor.xyz * textureLod(ssaotex, texCoord, 0.0).x;
    fragColor = vec4(_1352.x, _1352.y, _1352.z, fragColor.w);
    vec3 param = voxpos;
    vec3 _1371 = sampleLight(p, n, v, dotNV, pointPos, pointCol, albedo, metrough.y, occspec.y, f0, voxels, param);
    vec3 _1374 = fragColor.xyz + _1371;
    fragColor = vec4(_1374.x, _1374.y, _1374.z, fragColor.w);
}
